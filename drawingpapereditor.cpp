...
void QtDrawingPaperEditor::enterEvent(QEnterEvent *event) {
    Q_UNUSED(event)
}
...
void QtDrawingPaperEditor::wheelEvent(QWheelEvent *event) {
    ...
    if (!numPixels.isNull()) {
        OnWheelValueChanged(event->position().toPoint(),numPixels);
    } else if (!numDegrees.isNull()) {
        QPoint numSteps = numDegrees / 15;
        OnWheelValueChanged(event->position().toPoint(), numSteps);
    }
    event->accept();
}
